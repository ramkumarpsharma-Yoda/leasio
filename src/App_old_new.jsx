...
const BookingModal = ({ listing, onClose, onBooked, myAddress, setMyAddress }) => {
  ...
  const [bookForm, setBookForm] = useState({ date: "", slot: "", customHours: 2, hours: 2, qty: 1, days: 1, mode: "self", deliverySlot: "", renterAddress: "", renterName: "", renterPhone: "" });
  const [preDepPaid, setPreDepPaid] = useState(false);

  if (!listing) return null;
  const lt = listing.listingType;

  // ── deposit = booking amount for venue/service ──
  const calcTotal = () => {
    ...
    if (lt === "venue") {
      ...
      if (bookForm.slot === "Custom Hours") return listing.priceHour * bookForm.customHours;
      return listing.priceHour * (bookForm.slot.includes("Morning") || bookForm.slot.includes("Evening") ? 6 : 5);
    }
  ...
  // For items: use listing deposit. For venue/service: deposit = booking total
  const dep = lt === "item" ? (listing.deposit || 0) : total;
  const preDep = lt === "item" ? preBookDep(dep) : dep; // venue/service: pay full booking amount upfront
  ...
  const phoneValid = bookForm.renterPhone.replace(/\D/g, "").length === 10;
  ...
  return (
    <Modal onClose={onClose} maxW={520}>
      ...
      {step === 1 && <>
        ...
        {lt === "venue" && <>
          ...
          <div style={{ marginTop: 12 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#9CA3AF", letterSpacing: .7, marginBottom: 8, textTransform: "uppercase" }}>Select Booking Type</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {SLOTS.map(slot => {
                ...
                return (
                  <div key={slot}>
                    <button disabled={booked} onClick={() => setBookForm(f => ({ ...f, slot }))}
                    ...
                    {/* Custom hours picker shown inline when selected */}
                    {slot === "Custom Hours" && bookForm.slot === "Custom Hours" && (
                      <div style={{ background: "#0E1016", borderRadius: "0 0 9px 9px", padding: "10px 14px", marginTop: -1, border: "1.5px solid #F59E0B", borderTop: "none" }}>
                        <div style={{ fontSize: 11, fontWeight: 700, color: "#9CA3AF", marginBottom: 8, textTransform: "uppercase" }}>How many hours? ({fmt(listing.priceHour)}/hr)</div>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        {total > 0 && (
          <div style={{ background: "#0E1016", borderRadius: 10, padding: 12, marginTop: 14 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", paddingTop: 8, marginTop: 4, borderTop: "1px solid #252830" }}>
              ...
              <span style={{ fontWeight: 900, fontSize: 16, color: "#10B981" }}>{fmt(total + (lt === "item" ? dep : 0))}</span>
            </div>
          </div>
        )}
      </>}

      {/* STEP 2: Delivery mode — only for items */}
      {step === 2 && lt === "item" && <>
        <InfoBox color="#2563EB" icon="🚚" label="HOW WILL YOU RECEIVE THE ITEM?"></InfoBox>
      ...
      {/* STEP 2 for venue/service — skip delivery, go straight to identity */}
      {step === 2 && (lt === "venue" || lt === "service") && <>
        <InfoBox color="#6366F1" icon="📋" label="BOOKING SUMMARY">
          <Row l={lt === "venue" ? "Slot" : "Duration"} r={lt === "venue" ? bookForm.slot : `${bookForm.hours} hours`} />
          <Row l="Amount" r={fmt(total)} />
          <Row l="Advance deposit" r={fmt(dep)} color="#F59E0B" />
        </InfoBox>
        <InfoBox color="#F59E0B" icon="💡" label="HOW DEPOSIT WORKS FOR VENUE / SERVICE"
          sub="Your advance deposit equals the full booking amount. It is held securely by Leasio and released to the owner after your booking is confirmed. Full refund if owner cancels." />
      </>}

      {/* STEP 3: Identity */}
      {step === 3 && <>
        <InfoBox color="#7C3AED" icon="🪪" label="YOUR DETAILS ARE REQUIRED"
          sub="Your details are shared with the owner for coordination. This prevents anonymous transactions.">
        </InfoBox>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div>
            <Inp label="Mobile Number (10 digits)" value={bookForm.renterPhone}
              onChange={e => {
            ...
            {bookForm.renterPhone.length > 0 && bookForm.renterPhone.length < 10 && (
              <div style={{ color: "#EF4444", fontSize: 11, marginTop: 4 }}>⚠ Enter 10 digit mobile number</div>
            )}
            {bookForm.renterPhone.length === 10 && (
              <div style={{ color: "#10B981", fontSize: 11, marginTop: 4 }}>✓ Valid mobile number</div>
            )}
          </div>
          ...
          <InfoBox color="#F59E0B" icon="🔒" label="ADDRESS PRIVACY"
            sub="Your address is only shared with the owner and is never publicly visible." />
        </div>
      </>}
      ...
      {step === 4 && <>
        <div style={{ background: "#0E1016", borderRadius: 10, padding: 14, marginBottom: 14 }}>
          {lt === "item" ? <>
            ...
            <Row l="Remaining deposit (at handover)" r={fmt(remainDep)} />
            ...
            <Row l="Booking amount" r={fmt(total)} />
            <Row l="Advance deposit (pay now)" r={fmt(dep)} color="#F59E0B" />
            <div style={{ display: "flex", justifyContent: "space-between", paddingTop: 8, marginTop: 4, borderTop: "1px solid #252830" }}>
              <span style={{ fontWeight: 800 }}>You pay now</span>
              <span style={{ fontWeight: 900, color: "#10B981" }}>{fmt(dep)}</span>
            </div>
          </>}
        </div>
        <InfoBox color="#2563EB" icon="ℹ️" label="WHAT YOU GET AFTER PAYING">
          <div style={{ fontSize: 12, color: "#9CA3AF", lineHeight: 1.7 }}>
            ✓ Owner's full address / contact is revealed<br/>
            ✓ Owner is notified with your contact details<br/>
            {lt === "item" ? "✓ Pre-deposit is fully refunded if owner doesn't confirm within 4 hours" : "✓ Full refund if owner cancels your booking"}<br/>
            ✓ Booking slot is held for you
          </div>
        </InfoBox>
        <Btn variant="primary" style={{ width: "100%" }} onClick={payPreDeposit}>
          {`Pay ${fmt(Math.max(preDep, dep, 1))} & Unlock Owner Details`}
        </Btn>
      </>}

      {/* STEP 5: Address revealed */}
      {step === 5 && preDepPaid && <>
        <InfoBox color="#10B981" icon="✅" label="PAYMENT RECEIVED — OWNER DETAILS UNLOCKED" />
        <div style={{ background: "#0E1016", borderRadius: 12, padding: 16, marginBottom: 14 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#10B981", marginBottom: 10, letterSpacing: .5 }}>📍 OWNER / PROVIDER DETAILS (Only visible to you)</div>
          ...
          <div style={{ fontSize: 13, color: "#9CA3AF", marginBottom: 4 }}>📍 {listing.full_address || listing.locality + ", " + (listing.city || "Bengaluru")}</div>
        ...
        <InfoBox color="#7C3AED" icon="📋" label="YOUR BOOKING SUMMARY">
          <Row l="Listing" r={listing.title.slice(0, 28)} />
          {lt === "item" && bookForm.days > 1 && <Row l="Duration" r={`${bookForm.days} days`} />}
          {lt === "venue" && <Row l="Slot" r={bookForm.slot + (bookForm.slot === "Custom Hours" ? ` (${bookForm.customHours} hrs)` : "")} />}
          {lt === "service" && <Row l="Duration" r={`${bookForm.hours} hours`} />}
          <Row l="Amount" r={fmt(total)} />
          <Row l="Deposit paid" r={fmt(lt === "item" ? preDep : dep)} color="#10B981" />
          {lt === "item" && remainDep > 0 && <Row l="Balance deposit" r={fmt(remainDep)} color="#F59E0B" />}
        </InfoBox>
        <Btn variant="success" style={{ width: "100%" }} onClick={finalConfirm}>✓ Confirm Booking</Btn>
      </>}
          ...
          {step < 4 && (
            <Btn variant="primary" style={{ flex: 1 }} onClick={() => setStep(s => s + 1)}
              disabled={
                ...
                (step === 3 && (!bookForm.renterName || bookForm.renterPhone.length !== 10 || !bookForm.renterAddress))
              }>
              Continue →
            </Btn>
          )}
    ...
    <Modal onClose={onClose} maxW={500}>
      ...
      {step === 1 && <>
        ...
        <InfoBox color="#10B981" icon="🛒" label="SECURE PURCHASE — HOW IT WORKS"
          sub="Payment held by Leasio. Seller's full address revealed only after payment. You confirm receipt; funds released to seller." />
      ...
      {step === 2 && <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
        <InfoBox color="#7C3AED" icon="🪪" label="YOUR DETAILS REQUIRED"
          sub="Your name and address are shared with the seller so they know who is coming to collect." />
        <Inp label="Full Name" value={form.name} onChange={e => setForm(f => ({...f,name:e.target.value}))} placeholder="As per Aadhaar" />
        <div>
          <Inp label="Mobile Number (10 digits)" value={form.phone}
            onChange={e => {
              const val = e.target.value.replace(/\D/g, "").slice(0, 10);
      ...
      <div style={{ display:"flex", gap:8, marginTop:16 }}>
        ...
        {step === 2 && <Btn variant="success" style={{ flex:1 }} disabled={!form.name || !phoneValid || !form.address} onClick={confirm}>Pay {fmt(price)} & Unlock Seller Address</Btn>}
...
const LoginScreen = ({ onLogin }) => {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      if (error) throw error;
      onLogin(data.user);
    } catch (e) { setError('Signup failed: ' + e.message); }
    setLoading(false);
  };
    ...
    <div style={{ padding: 24, maxWidth: 780, margin: "0 auto" }}>
      ...
      <div style={{ background: "#13151C", border: "1px solid #1E2130", borderRadius: 16, padding: 24, marginBottom: 20 }}>
        ...
        <InfoBox color="#F59E0B" icon="🔒" label="ADDRESS PRIVACY SYSTEM"
          sub={lt === "item" ? `Full pickup address shown only after paying 20% pre-deposit (${fmt(preBookDep(item.deposit || 1000))}). Your address is also required for this booking.` : "Owner contact details revealed after booking payment is confirmed."} />
...
// ─── BROWSE VIEW ─────────────────────────────────────────────────────────────
const BrowseView = ({ listings, filtered, allCats, search, setSearch, locality, setLocality, filterLT, setFilterLT, filterSale, setFilterSale, filterCat, setFilterCat, setSelected, toast }) => {
...
export default function Leasio() {
  ...
  useEffect(() => {
    fetchListings()
      .then(data => setListings(data.length ? data : SEED))
      .catch(() => setListings(SEED));
  }, []);
  ...
  return (
    <div style={S.app}>
      ...
      {selected
        ...
        : view === "browse"
          ? <BrowseView listings={listings} filtered={filtered} allCats={allCats} search={search} setSearch={setSearch} locality={locality} setLocality={setLocality} filterLT={filterLT} setFilterLT={setFilterLT} filterSale={filterSale} setFilterSale={setFilterSale} filterCat={filterCat} setFilterCat={setFilterCat} setSelected={setSelected} toast={toast} />
          : view === "list"
